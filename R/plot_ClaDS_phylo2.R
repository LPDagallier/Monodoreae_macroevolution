plot_ClaDS_phylo2 <- function (phylo, rates, rates2 = NULL, same.scale = T, main = NULL, 
          lwd = 2, log = T, show.tip.label = F, ...) 
{
  ...
  if (is.null(rates2)) {
    ...
    if (same.scale) {
      ...
      par(mfrow = c(1, 2))
      col = round(((rates - min)/(max - min)) * 99) + 
        1
      plot(phylo, edge.color = Colors[col], show.tip.label = show.tip.label, 
           edge.width = lwd, ...)
      ...
      if (isTRUE(all.equal(rep(rates[1], length(rates)), 
        ...
        if (log) {
          ...
          image.plot(z = c(rates[1], 2 * rates[1]), 
                     col = Colors, horizontal = T, legend.only = T)
        }
      }
      else {
        ...
        if (log) {
          ...
          image.plot(z = c(min, max), col = Colors, 
                     horizontal = T, legend.only = T, axis.args = list(at = log(ticks), 
                                                                       labels = ticks))
        }
        else {
          image.plot(z = as.matrix(rates), col = Colors, 
