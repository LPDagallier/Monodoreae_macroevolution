...
Data from [Polissar *et al.* 2019](https://www.nature.com/articles/s41561-019-0399-2) and [Uno *et al.* 2016](https://www.pnas.org/content/113/23/6355).  
...
The model will not be able to fit the environmental data because the older environmental point is at 23.38 Myr whereas the older node in the Monodoreae phylogeny is 25.04 Myr old. In order to overcome this problem, I just add 1 data in the environmental dataset at 25.1 Myr which is the mean of the last more or less table dots.
...
```{r tropics2-percC4-BConst, eval = F}
source(file ="R/plot_fit_env2.R")
...
```{r tropics2-percC4-BTime, eval = F}
source(file ="R/plot_fit_env2.R")
...
```{r tropics2-percC4-BDConst, eval = F}
source(file ="R/plot_fit_env2.R")
f.lamb <-function(t,x,y){y[1]+0*t}
...
Plot the model:
```{r tropics2-percC4-BDConst-plot}
...
```{r tropics2-percC4-BTimeDConst, eval = F}
f.lamb <-function(t,x,y){y[1] * exp(y[2] * t)}
f.mu<-function(t,x,y){y[1]+0*t}
lamb_par<-c(0.5, 0.1)
mu_par<-c(0.01)
...
Plot the model:
```{r tropics2-percC4-BTimeDConst-plot}
...
```{r tropics2-percC4-BConstDTime, eval = F}
f.lamb <-function(t,x,y){y[1]+0*t}
...
Plot the model:
```{r tropics2-percC4-BConstDTime-plot}
...
```{r tropics2-percC4-BDTime, eval = F}
f.lamb <-function(t,x,y){y[1] * exp(y[2] * t)}
f.mu<-function(t,x,y){y[1] * exp(y[2] * t)}
lamb_par<-c(0.1, 0.1)
mu_par<-c(0.01, 0.01)
...
```{r tropics2-percC4-BPercC4, eval = F}
...
Plot the model:
```{r tropics2-percC4-BPercC4-plot}
...
```{r tropics2-percC4-BPercC4DConst, eval = F}
f.lamb <-function(t,x,y){y[1] * exp(y[2] * x)}
...
Plot the model:
```{r tropics2-percC4-BPercC4DConst-plot}
...
```{r tropics2-percC4-BConstDPercC4, eval = F}
...
Plot the model:
```{r tropics2-percC4-BConstDPercC4-plot}
...
```{r tropics2-percC4-BDPercC4, eval = F}
f.lamb <-function(t,x,y){y[1] * exp(y[2] * x)}
f.mu<-function(t,x,y){y[1] * exp(y[2] * x)}
lamb_par<-c(0.1, 0.01)
mu_par<-c(0.1, 0.01)
...
Plot the model:
```{r tropics2-percC4-BDPercC4-plot}
...
The best fitting model appear to be the Birth Death model with speciation and extinction both varying exponentially with C4 fraction.  
...
```{r tropics2-percC4-plot-best-model, message=F}
